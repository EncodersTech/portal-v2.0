        ...
        <div v-else-if="items !== null && items.count > 0">
            <div v-if="items[current.body].length > 0">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr style="font-size:12px;" v-for="item in items[current.body]">
                            <td>
                            ...
                            <td>
                                <span v-if="item.meet !== null">{{ item.meet.name }}</span>
                            ...
                            <td>{{ item.readable.last_updated }}</td>
                            <td>
    ...
    methods: {
        ...
        showNext() {
            if (this.current.body !== null) {
                this.current.sanction = this.next(
                    this.items[this.current.body],
                    this.current.sanction
                );
                this.shown = this.items[this.current.body][
                    this.current.sanction
                ];
            }
        },
        ...
        showPrevious() {
            if (this.current.body !== null) {
                this.current.sanction = this.prev(
                    this.items[this.current.body],
                    this.current.sanction
                );
                this.shown = this.items[this.current.body][
        ...
        initialize() {
            try {
                    ...
                    if (this.gyms.hasOwnProperty(g)) {
                        ...
                        for (const s in gym.usag_sanctions) {
                            if (gym.usag_sanctions.hasOwnProperty(s)) {
                                ...
                                sanction.readable.timestamp = sanction.timestamp.format(
                                    "MM/DD/YYYY h:m:s a"
                                );

                                sanction.created_at = Moment(
                                    sanction.created_at
                                );
                                sanction.readable.created_at = sanction.created_at.format(
                                    "MM/DD/YYYY h:m:s a"
                                );

                                sanction.updated_at = Moment(
                                    sanction.updated_at
                                );
                                sanction.readable.updated_at = sanction.updated_at.format(
                                    "MM/DD/YYYY h:m:s a"
                                );
                                ...
                                if (
                                    !usag_sanctions.hasOwnProperty(item.number)
                                ) {
                                    usag_sanctions[item.number] = {
                                        ...
                                        readable: {
                                            last_updated: sanction.timestamp.format(
                                                "MM/DD/YYYY h:m:s a"
                                            )
                                        },
                                        url:
                                    ...
                                    if (
                                        sanction.timestamp >
                                        usag_sanctions[item.number].last_updated
                                    ) {
                                        usag_sanctions[
                                            item.number
                                        ].last_updated = sanction.timestamp;
                                        usag_sanctions[
                                            item.number
                                        ].readable.last_updated = sanction.timestamp.format(
                                            "MM/DD/YYYY h:m:s a"
                                        );
                                    }
                                }
                                ...
                                switch (sanction.action) {
                                    ...
                                    case this.constants.sanctions.actions
                                        .UPDATE:
                                        usag_sanctions[
                                            item.number
                                        ].has_update = true;
                                    ...
                                    case this.constants.sanctions.actions
                                        ...
                                        usag_sanctions[
                                            item.number
                                        ].has_vender_change = true;
                                        break;
                                }

                                usag_sanctions[item.number].items.push(
                                    sanction
                                );
                            }
                        }
                ...
                this.errorMessage =
                    "Something went wrong while processing your sanction notification." +
                    "Please contact us.<br/>" +
                    '<span class="small"><strong>Error details:</strong> ' +
                    error +
                    "</span>";
            }
        }
    ...
    beforeMount() {
        ...
        axios
            ...
            .catch(error => {
                ...
                this.errorMessage =
                    "Failed to load sanction notification. " + msg;
            })
            .finally(() => {
